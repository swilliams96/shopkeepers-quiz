...
class Item:
    ...
    def getWrongAnswers(self):
        ...
        for r in range(0, count):
            x = 0
            while x == 0 and result.find(str(self.cost + x*factor)) != -1:
    ...
    for item in items:
        if item.find('a'):
            ...
            if len(split) <= 1:                 # Item has no cost/is not purchasable so skip
                continue
            if split[0].find('Aegis') != -1:    # Items after the aegis are event/temporary items
